  ...
  <article :class="['recommend-card', { compact }]">
    ...
    <div class="recommend-actions">
      ...
      <button class="fold-btn" :aria-expanded="tagsOpen" @click="tagsOpen = !tagsOpen">
        <span>查看标签（{{ item.report_tags?.length ?? ((item.matched_tags?.length || 0) + (item.unmatched_tags?.length || 0)) }}）</span><b>{{ tagsOpen ? '⌃' : '⌄' }}</b>
      </button>
      <button v-if="allowRelated" class="related-btn" @click="emit('related', item)">关联案例</button>
    </div>
    <div v-if="tagsOpen" class="report-tag-list">
      <article v-for="tag in item.report_tags || []" :key="tag.name" class="report-tag-item">
        <b>{{ tag.name }}</b>
        <p>{{ tag.value?.trim() || '—' }}</p>
      </article>
      <template v-if="!item.report_tags?.length">
        <span v-for="tag in item.matched_tags" :key="`hit-${tag.name}`" class="tag success">✓ {{ tag.name }}：{{ tag.report_value }}</span>
        <span v-for="tag in item.unmatched_tags" :key="`miss-${tag.name}`" class="tag muted">{{ tag.name }}：{{ tag.report_value }}</span>
      </template>
    </div>
