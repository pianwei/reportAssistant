...
数据统计会从当前启动实例的全部报告生成全量标签 JSON 快照。报告总数、标签总数和分布由 SQLite 精确聚合；“科技企业的报告有哪些”“授信额度大于200万的报告有哪些”等条件统计由当前激活模型理解非规范标签，后端校验模型返回的报告 ID，并对可识别金额条件再次进行单位换算和数值校验。模型不可用时使用本地保守匹配，不会编造报告。

推荐会直接返回 Top 3，多维筛选会立即返回全部 AND 命中结果，不再通过追问卡细化条件。用户可直接发送新的问题或条件，助手回答后会以气泡形式展示3个衍生问题。关联案例通过统一聊天接口调用：
