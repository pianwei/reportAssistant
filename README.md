...
数据统计会从当前启动实例的全部报告生成列式全量标签 JSON 快照：报告 ID 数组与每类标签值数组按位置一一对应。报告总数、标签总数和分布由 SQLite 精确聚合；所有条件统计统一由当前激活模型一次分析完整快照并返回全部匹配报告 ID，后端校验 ID 后回填报告信息，不接受快照中不存在的报告 ID。模型连接、上下文或结构化输出失败时返回明确错误，不会把分析失败伪装成零结果。
